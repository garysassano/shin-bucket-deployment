  ...
  const selections = [...shinGroups.values()].flatMap((shinRecords) => {
    ...
    const awsRecords = records.filter(
      (record) =>
        ...
        // AWS samples omit `parallel`; Shin parallelism is not an upstream input.
        // This must stay inside the conjunction — hoisting it out matches every
        // AWS row from every profile and memory size.
        record.parallel === undefined,
    );
