...
import { parseBenchmarkRunOptions } from "./config";
...
export function selectValidatedBenchmarkRun(args: {
  ...
  const selected = selectBenchmarkRun(
    args.records
      ...
      .filter((record) => benchmarkMethodologyVersion(record) === args.methodologyVersion)
      .filter((record) => args.methodologyVersion === 1 || record.gitDirty === false),
    args.runId,
  );
