...
Every Shin benchmark sets `failureDiagnostics: "detailed"`; upstream AWS CDK samples record the corresponding evidence field as not applicable. Benchmark measurements therefore exercise the conservative, fully observable Shin mode, including detailed failed-attempt bookkeeping. Normal production deployments use `"standard"` and retain only the basic aggregate retry, wire, and failure counters.
