...
| Invocation-wide memory cap  | Central-directory planning and every source archive share a source-memory budget capped at half the provider's actual Lambda memory by default; destination cleanup retains manifest metadata plus stale keys capped at 4 MiB each for the current and previous namespaces, then re-lists one S3 page at a time if a cap overflows. |
...
interface ShinBucketDeploymentProps {
  ...
  readonly sourceProcessing?: {
    ...
    readonly maxUncompressedEntryBytes?: number; // Default: 1 GiB
...
| `deployment.objectKeys`        | Bound source archive keys (`SourceObjectKeys`), not extracted destination keys, requested lazily from the custom-resource response. |
