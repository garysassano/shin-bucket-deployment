...
function verifyConsumerInstall(tarball, workDir) {
  ...
  writeFileSync(
    ...
    [
      ...
      `import { DEFAULT_FAILURE_DIAGNOSTICS, DEFAULT_MAX_PARALLEL_TRANSFERS, DEFAULT_PROVIDER_LAMBDA_MEMORY_SIZE_MIB, DestinationWriteRetryJitter, FailureDiagnostics, ProviderScope, ShinBucketDeployment, type ShinBucketDeploymentAdvancedTransferTuning, type ShinBucketDeploymentDestination, type ShinBucketDeploymentLocalBuildOptions, type ShinBucketDeploymentProps, type ShinBucketDeploymentProviderLambdaOptions, type ShinBucketDeploymentSourceProcessingOptions, type ShinBucketDeploymentTransferOptions, Source, ValidationError } from "${packageName}";`,
      "",
      "const app = new App();",
      'const stack = new Stack(app, "ConsumerStack");',
      'const bucket = new Bucket(stack, "Bucket");',
      "const destination: ShinBucketDeploymentDestination = { bucket };",
      "const sourceProcessing: ShinBucketDeploymentSourceProcessingOptions = { extract: true };",
      ...
      'if (DEFAULT_PROVIDER_LAMBDA_MEMORY_SIZE_MIB !== 1024) throw new Error("invalid provider memory default export");',
      'if (DEFAULT_MAX_PARALLEL_TRANSFERS !== 32) throw new Error("invalid transfer default export");',
  ...
  const synthLines = [
    ...
    `const { DEFAULT_FAILURE_DIAGNOSTICS, DEFAULT_MAX_PARALLEL_TRANSFERS, DEFAULT_PROVIDER_LAMBDA_MEMORY_SIZE_MIB, DestinationWriteRetryJitter, FailureDiagnostics, ProviderScope, ShinBucketDeployment, Source, ValidationError } = require("${packageName}");`,
    "",
    "assert.equal(DEFAULT_PROVIDER_LAMBDA_MEMORY_SIZE_MIB, 1024);",
    "assert.equal(DEFAULT_MAX_PARALLEL_TRANSFERS, 32);",
  ...
  writeFileSync(
    ...
    [
      ...
      'import { Bucket } from "aws-cdk-lib/aws-s3";',
      `import { DEFAULT_FAILURE_DIAGNOSTICS, DEFAULT_MAX_PARALLEL_TRANSFERS, DEFAULT_PROVIDER_LAMBDA_MEMORY_SIZE_MIB, DestinationWriteRetryJitter, FailureDiagnostics, ProviderScope, ShinBucketDeployment, Source, ValidationError } from "${packageName}";`,
      "",
      "assert.equal(DEFAULT_PROVIDER_LAMBDA_MEMORY_SIZE_MIB, 1024);",
      "assert.equal(DEFAULT_MAX_PARALLEL_TRANSFERS, 32);",
      "assert.equal(DEFAULT_FAILURE_DIAGNOSTICS, FailureDiagnostics.STANDARD);",
