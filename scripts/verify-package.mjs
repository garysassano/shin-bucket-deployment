...
const expectedLibraryFiles = [
  ...
  "package/lib/local-build.d.ts",
  "package/lib/local-build.js",
...
function verifyConsumerInstall(tarball, workDir) {
  ...
  writeFileSync(
    ...
    [
      ...
      `import { DEFAULT_FAILURE_DIAGNOSTICS, DEFAULT_MAX_COMPRESSION_RATIO, DEFAULT_MAX_UNCOMPRESSED_ENTRY_BYTES, DEFAULT_PROVIDER_LAMBDA_MEMORY_SIZE_MIB, DEFAULT_TRANSFER_MAX_CONCURRENCY, DestinationWriteRetryJitter, FailureDiagnostics, ProviderSharing, ShinBucketDeployment, type ShinBucketDeploymentAdvancedTransferTuning, type ShinBucketDeploymentDestination, type ShinBucketDeploymentBundlingCommandHooks, type ShinBucketDeploymentBundlingDockerOptions, type ShinBucketDeploymentBundlingOptions, type ShinBucketDeploymentLocalBuildOptions, type ShinBucketDeploymentProps, type ShinBucketDeploymentProviderLambdaOptions, type ShinBucketDeploymentSourceProcessingOptions, type ShinBucketDeploymentTransferOptions, Source, ValidationError } from "${packageName}";`,
      "",
      "const app = new App();",
      'const stack = new Stack(app, "ConsumerStack");',
      'const bucket = new Bucket(stack, "Bucket");',
      "const destination: ShinBucketDeploymentDestination = { bucket };",
      "const sourceProcessing: ShinBucketDeploymentSourceProcessingOptions = { extract: true, maxUncompressedEntryBytes: DEFAULT_MAX_UNCOMPRESSED_ENTRY_BYTES, maxCompressionRatio: DEFAULT_MAX_COMPRESSION_RATIO };",
      "const hooks: ShinBucketDeploymentBundlingCommandHooks = { beforeBundling: () => [], afterBundling: () => [] };",
      "const docker: ShinBucketDeploymentBundlingDockerOptions = { network: 'host' };",
      "const bundling: ShinBucketDeploymentBundlingOptions = { commandHooks: hooks, dockerOptions: docker };",
      "const buildOptions: ShinBucketDeploymentLocalBuildOptions = { projectPath: 'rust', bundling };",
      "void buildOptions;",
      "const localBuild: ShinBucketDeploymentLocalBuildOptions | undefined = undefined;",
      ...
      "  destinationWriteRetry: { jitter: DestinationWriteRetryJitter.FULL },",
      "};",
      "const transfer: ShinBucketDeploymentTransferOptions = { advancedTuning };",
      'if (DEFAULT_PROVIDER_LAMBDA_MEMORY_SIZE_MIB !== 2048) throw new Error("invalid provider memory default export");',
      'if (DEFAULT_TRANSFER_MAX_CONCURRENCY !== 64) throw new Error("invalid transfer default export");',
      'if (DEFAULT_MAX_UNCOMPRESSED_ENTRY_BYTES !== 1073741824) throw new Error("invalid entry-size default export");',
      'if (DEFAULT_MAX_COMPRESSION_RATIO !== 100) throw new Error("invalid compression-ratio default export");',
      'if (DEFAULT_FAILURE_DIAGNOSTICS !== FailureDiagnostics.STANDARD) throw new Error("invalid diagnostics default export");',
      "const minimal: ShinBucketDeploymentProps = {",
      '  sources: [Source.asset("site")],',
      "  destination,",
      "};",
      'new ShinBucketDeployment(stack, "Deploy", minimal);',
      "const complete: ShinBucketDeploymentProps = {",
      '  sources: [Source.data("complete.txt", "complete")],',
      '  destination: { bucket, keyPrefix: "complete" },',
      "  sourceProcessing,",
      "  providerLambda,",
      "  transfer,",
      "};",
      'new ShinBucketDeployment(stack, "CompleteDeploy", complete);',
      "app.synth();",
  ...
  const synthLines = [
    ...
    "assert.equal(DEFAULT_TRANSFER_MAX_CONCURRENCY, 64);",
    "assert.equal(DEFAULT_FAILURE_DIAGNOSTICS, FailureDiagnostics.STANDARD);",
    `const app = new App({ outdir: ${JSON.stringify(join(workDir, "cdk.out-cjs"))} });`,
    'const stack = new Stack(app, "ConsumerStack");',
    'const bucket = new Bucket(stack, "Bucket");',
    'new ShinBucketDeployment(stack, "Deploy", {',
    '  sources: [Source.asset("site")],',
    '  destination: { bucket, keyPrefix: "shared" },',
    "});",
    ...
    "  },",
    "  transfer: {",
    "    advancedTuning: { destinationWriteRetry: { jitter: DestinationWriteRetryJitter.FULL } },",
    "  },",
    "});",
    ...
    "    destination: { bucket: validationBucket },",
    ...
    'assert.equal(validationError.code, "ShinBucketDeploymentCloudFrontDistributionRequired");',
  ...
  writeFileSync(
    ...
    [
      ...
      "}",
      "app.synth();",
      'const publicStack = new Stack(new App(), "PublicSubnetStack");',
      'const publicVpc = new Vpc(publicStack, "Vpc", { maxAzs: 2, natGateways: 0 });',
      "const publicSubnet = publicVpc.publicSubnets[0];",
      'const importedPublic = Subnet.fromSubnetAttributes(publicStack, "ImportedPublic", {',
      "  subnetId: Token.asString({ resolve: () => publicStack.resolve(publicSubnet.subnetId) }),",
      "  availabilityZone: publicSubnet.availabilityZone, routeTableId: publicSubnet.routeTable.routeTableId,",
      "});",
      "function publicDeploy(id, subnet) {",
      "  return new ShinBucketDeployment(publicStack, id, {",
      '    sources: [Source.data("index.txt", "public subnet")],',
      '    destination: { bucket: new Bucket(publicStack, id + "Bucket") },',
      "    providerLambda: { vpc: publicVpc, vpcSubnets: { subnets: [subnet] } },",
      "  });",
      "}",
      'publicDeploy("Imported", importedPublic);',
      'assert.throws(() => publicDeploy("ActualPublic", publicSubnet), error => error.code === "ShinBucketDeploymentProviderPublicSubnet");',
      "",
    ].join("\n"),
  );
