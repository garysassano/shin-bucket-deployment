...
function verifyConsumerInstall(tarball, workDir) {
  ...
  writeFileSync(
    ...
    [
      ...
      `import { DestinationWriteRetryJitter, FailureDiagnostics, ProviderScope, ShinBucketDeployment, Source, ValidationError } from "${packageName}";`,
      "",
      "const app = new App();",
      ...
      "  advancedRuntimeTuning: { destinationWriteRetry: { jitter: DestinationWriteRetryJitter.FULL } },",
      "});",
      "app.synth();",
  ...
  const synthLines = [
    ...
    `const { DestinationWriteRetryJitter, FailureDiagnostics, ProviderScope, ShinBucketDeployment, Source, ValidationError } = require("${packageName}");`,
    "",
