...
> [!IMPORTANT]
> Streaming marker replacement and invocation memory planning each have complete local and AWS correctness evidence at their recorded pre-integration commits. Refreshed verification of the combined branch is pending. Raw AWS output and identifiers remain outside git.
...
| Current verification baseline | Pre-integration snapshots at `d8b40a5` for marker streaming and `72e2fcf` for invocation memory planning |
| Current verification category | Deterministic local gates plus separate deployed AWS end-to-end snapshots; combined rerun pending |
| Latest current runs | `2026-07-14-local-marker-follow-up` and `2026-07-13-aws-invocation-memory-planning` |
| AWS status for combined code | Pending refreshed verification after integration |
...
| --- | --- | --- | --- |
| P0 | Rust provider tests | The marker baseline passed 128 tests with one credential-gated integration test ignored; the memory-planning baseline passed 130 with one ignored. Together they cover deterministic bounded marker streaming, fair aggregate source permits, cancellation release, actual-memory authority, checked tuning validation, manifest-only destination metadata, and indexed interval lookup. | Local pass on each pre-integration baseline; combined rerun pending |
| P0 | TypeScript and release-script tests | The recorded baselines passed 89 and 88 Vitest tests respectively plus five Node release tests each, covering construct synthesis, validation, benchmark assets, and lifecycle behavior. | Local pass on each pre-integration baseline; combined rerun pending |
| P0 | Build and static checks | Both recorded baselines passed their TypeScript, package, Biome, Rust, dependency, workflow, synthesis, and provider-build gates. | Local pass on each pre-integration baseline; combined rerun pending |
...
| P0 | Invocation memory and destination planning | Local tests bound two independent archives with one fair permit pool and prove cancellation wakes waiters. All 18 captured AWS deployment summaries read the actual 1024 MiB Lambda size, used an exact 536,870,912-byte source cap, finished at zero resident bytes, and stayed within the cap. Thirteen summaries used multiple sources; ten observed aggregate high-water above the largest single archive. Destination metadata never exceeded listed objects, stale cleanup was observed, and page high-water was 8. | Local and AWS pass |
...
- The invocation-global cap governs retained source blocks, not every process allocation. Runtime, SDK, transfer-body, ZIP metadata, and marker buffers use the other half of Lambda memory and are still bounded by their own concurrency and object-size rules.
- Destination metadata is O(manifest keys), and stale cleanup is O(one S3 page); the deployment manifest itself remains proportional to planned object count.
