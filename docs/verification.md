...
> destination lifecycle contract in AWS. CloudFormation deadline/protocol fault
> paths have a current deterministic local snapshot. The remaining full-suite
> scenarios and packaged x86_64 runtime path still need a current AWS rerun.
...
| Latest verification baseline | `2c7c6d6` (`fix: harden CloudFormation protocol reliability`) |
| Region | Local protocol/lifecycle gates plus targeted AWS lifecycle verification in `eu-central-1`; previous full AWS suite in `ap-southeast-2` |
| Latest verification runs | `2026-07-11-local-protocol-reliability`, `2026-07-11-targeted-lifecycle-aws`, and `2026-07-11-local-lifecycle-contract` |
...
| --- | --- | --- | --- |
| P0 | Rust provider tests | CloudFormation callback 2xx/4xx/5xx/connection/timeout behavior, response-size boundaries, deadline cancellation/drain, `ResourceType`, CloudFront path/poll limits, lifecycle authorization, archive planning, marker replacement, and transfer retry helpers. | Pass as of 2026-07-11 local protocol gates |
...
| P0 | Build and lint | TypeScript build/typecheck/lint, Rust fmt/check/all-feature clippy/tests, package smoke test, npm audit, cargo audit, cargo deny, actionlint, and Taplo. | Pass as of 2026-07-11 local protocol gates |
| P0 | Scenario synthesis | All 19 default `ShinBucketDeployment` verification scenarios synthesize locally. | Pass as of 2026-07-11 local protocol gates |
