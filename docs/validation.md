...
| P0 | S3 algorithm integration | Ignored Rust test generates ZIP archives, uploads them to a temporary S3 source bucket, runs `deploy()` locally against S3 clients, validates extracted destination objects, marker replacement, prune, unchanged redeploy, and cleanup. | `AWS_PROFILE=<profile> AWS_REGION=<region> RBD_AWS_INTEGRATION_FILE_COUNT=2500 cargo test --manifest-path rust/Cargo.toml s3::aws_integration_tests::deploys_generated_zip_archives_from_s3_to_s3 -- --ignored --nocapture` | 2026-05-02 | Pass; local max RSS observed below 128 MiB with 2,500 generated files |
