...
The September 2026 refresh updates the provider lockfile after checking published crate sources and upstream release notes. It changes CRC processing, compression-buffer validation, SDK signing behavior, and Lambda Runtime API responses, so performance acceptance remains pending until comparable before/after measurements and upstream AWS CDK controls are committed on `main`.
...
The maintainer owns deployed verification and benchmark evidence for this refresh. The selected comparison uses the existing `readme-snapshots` configuration: `tiny-many`, `mixed`, and `large-few` at both 1024 MiB / 32 transfers and 2048 MiB / 64 transfers, all four phases, five repetitions, and detailed telemetry. The initial before attempt at clean `main` commit `32cbbfe074560ac43df240bbfe21cb16192340f1` encountered a runner provenance race with pnpm temporary files and has an incomplete repetition, so it cannot establish acceptance. A separate runner correction precedes a complete repeated baseline from clean `main` before this dependency change; the after provider must be the exact clean implementation squash commit on `main`. Cleanup and complete validated ledgers must be published before performance acceptance. Callback/runtime changes and the [shared-handler replacement](breaking-changes.md#reviewed-runtime-dependencies-change-prebuilt-shared-handler-identity) also require the applicable deployed correctness check; benchmark rows are not correctness evidence.
