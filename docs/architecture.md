...
The split exists so network fetches inside planning (metadata heads, directory GETs, catalog GETs) can be told apart from CPU cost: those round-trips are what make planning look CPU-heavy while actually being waits. All five sub-timings accumulate per archive at microsecond resolution and are converted to whole milliseconds once, at snapshot time, rounding to the nearest millisecond — accumulated sub-millisecond spans report 1 ms or more instead of the constant 0 that per-call truncation produced, and a stage that never ran still reports 0. `planSourceHeads` is exclusive: the metadata `HeadObject` awaits run in the per-source loop, outside the catalog/directory/entries/validation buckets, so `planSourceHeads + planCatalog + planDirectory + planEntries + planValidation` partitions the instrumented planning stages exactly. The wall-clock spans sum meaningfully only while source planning is sequential (one archive at a time, as it is today); if archive planning becomes concurrent, summing them would double-count overlapping time and the accounting must move to per-archive self-time spans instead.

The five sub-timings cover the ZIP planning stages plus the metadata heads but not every part of `plan`: the residual (`plan` minus the sum of the five) is the request length checks, source-budget setup, filter compilation, manifest insertion in copy mode, `collect_zip_entry_plans`, and the planned-entries accounting. In copy mode (`extract:false`) no ZIP stages run, so the four ZIP sub-timings report 0, `planSourceHeads` carries the metadata `HeadObject` round-trips, and `plan` is those heads plus the CPU-only residual; the zeros are the honest answer there, not an omission.
...
| `phaseMs.transferPrepare`         | Cumulative time inside `prepare_zip_entry_upload`: the comparison pass (source read, decode, hash, marker replacement). | Attribute CPU work done before the destination PUT.                                               |
...
| `phaseMs.transferSourceFetchWait` | Cumulative time transfer entry readers spend awaiting source block data, recorded when each in-flight fetch resolves.   | Separate source-network waits from CPU; read it as an overlay, not a sibling bucket.              |

These five are NOT a wall-clock partition of `transfer`, and `transferTaskTotal` is not a wall-clock span: up to `maxParallelTransfers` ZIP-entry tasks run concurrently, so these counters are sums across tasks and can legitimately exceed the `transfer` wall clock, often by a large multiple. The relationships that do hold are `transferPrepare + transferPutWait ≈ transferTaskTotal` (the remainder is scheduler and task overhead), and `transferSourceFetchWait` is an overlay rather than a sibling bucket: source fetches happen inside both the prepare span (comparison and marker-planning reads) and, for streaming upload bodies that generate their body during the PUT, inside the put span, so it must never be added to the other three. The intended attribution: destination-network-bound shows as `transferPutWait` dominating with a small `transferSourceFetchWait`; source-network-bound shows as `transferSourceFetchWait` dominating; CPU-bound shows as `transferTaskTotal - transferPutWait - transferSourceFetchWait` dominating. Only ZIP-entry transfer tasks are instrumented: direct-copy tasks (`copy_source_object`) are a separate path and never contribute to `transferTaskTotal`, so on copy-heavy workloads `transferTaskTotal` is much smaller than `transfer`. The plan-phase embedded-catalog read also flows through the same entry reader but passes no stats handle, so it never counts as a transfer fetch wait. All five transfer members accumulate at microsecond resolution and convert to whole milliseconds exactly once at snapshot time, like the plan sub-timings.
