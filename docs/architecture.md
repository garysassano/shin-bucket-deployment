...
- The cataloged wrapper materializes into a deterministic per-source staging directory (kept across binds so CDK's process-global `AssetStaging` cache can hit on repeated synthesis), refreshes its contents on every bind, then delegates ZIP and ZIP64 creation to CDK; the catalog changes the staged asset hash compared with upstream packaging.
