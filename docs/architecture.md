...
Concurrency is a workload limit, not a throughput target. On the measured mixed workload, raising `transfer.maxConcurrency` to 128 slowed cold-create at both 1024 MiB and 2048 MiB compared with the measured 32- and 64-task configurations. That observation came from the since-superseded `3a1fe594` run at one repetition each; its shin rows were archived with the transfer sub-timings schema change, so the committed ledger holds no 2048/128 rows and can no longer reproduce the figure. The 2026-08-13 exploratory sweep then measured `large-few` cold-create at 4096 MiB / 128 as about 32% faster than 2048 MiB / 64 in the same run by provider duration (1.379 s to 0.939 s); end-to-end `cdkDeploySeconds` moved only ~4% (57.4 s to 55.0 s) on the same pair, because provider time is a small share of a CDK deploy. The ceiling therefore appears to track available memory rather than holding at every memory size, but the win is in provider duration, not in what an operator waits for. That sweep varied memory and concurrency together on one repetition each and does not isolate either axis, and none of this evidence proves that 64 is optimal for every source shape. Resolved values above 64 therefore remain valid but emit `ShinBucketDeploymentHighTransferConcurrency`; acknowledge that warning only after a representative benchmark shows a benefit and provider diagnostics confirm that `inFlightHighWater` is useful work rather than source starvation or destination retries/throttling. Unresolved values are left to provider validation and cannot receive synthesis-time guidance.
...
The default provider Lambda memory is 2048 MiB. The runtime reads `AWS_LAMBDA_FUNCTION_MEMORY_SIZE`; the custom-resource payload is not trusted as the memory source. Half of the actual memory becomes the default invocation-global ZIP planning and source-block cap, leaving the other half for runtime, SDK, transfer, marker, and manifest allocations. The default moved from 1024 MiB with 32 transfers to 2048 MiB with 64 on canonical five-repetition evidence: 2048/64 measured 31-44% faster cold-create than 1024/32 on every canonical profile, at a peak usage far below either allocation. The earlier 1024 MiB default came from single-sample exploratory measurements that were never a sufficient basis for the choice. Note that Lambda bills memory x duration, so the faster configuration is not automatically the cheaper one; cost-sensitive deployments that tolerate slower deploys should set `providerLambda.memorySize` explicitly.
