...
- CloudFront invalidation paths: each concrete path must start with `/`, be at most 4,000 Unicode characters, and contain no control characters or `~`; a batch must contain at most 3,000 paths, of which at most 15 may be wildcard paths (a path whose final character is `*`). Enforced at synthesis and re-checked by the provider for protocol callers.
