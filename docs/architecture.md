...
| `destination-move-matrix-initial` / `destination-move-matrix-updated` | `scenarios/apps/lifecycle/destination-move-matrix-initial-app.ts`, `scenarios/apps/lifecycle/destination-move-matrix-updated-app.ts` | Self-verifying child-to-parent, parent-to-child, sibling-prefix, and cross-bucket moves with destructive Delete enabled, previous cleanup explicitly disabled and enabled, and exact initial/current/retained/deleted object-state assertions. |
