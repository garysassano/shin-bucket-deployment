...
| Field                    | Meaning                                                                                                                                                                                                                | Use when debugging                                                     |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `phaseMs.plan`           | Total wall time across all source planning: request length checks, source-budget setup, filter compilation, per-source metadata `HeadObject` fetches, archive planning, deployment preflight, and zip-plan collection. | Compare the whole planning phase with the sum of its four sub-timings. |
...
The split exists so network fetches inside planning (directory GETs, catalog GETs) can be told apart from CPU cost: those round-trips are what make planning look CPU-heavy while actually being waits. The four sub-timings accumulate per archive at microsecond resolution and are converted to whole milliseconds once, at snapshot time, rounding to the nearest millisecond — accumulated sub-millisecond spans report 1 ms or more instead of the constant 0 that per-call truncation produced, and a stage that never ran still reports 0. They are wall-clock spans, so their sum is meaningful only while source planning is sequential (one archive at a time, as it is today); if archive planning becomes concurrent, summing them would double-count overlapping time and the accounting must move to per-archive self-time spans instead.

The four sub-timings cover the ZIP planning stages but not every part of `plan`: the residual (`plan` minus the sum of the four) is the request length checks, source-budget setup, filter compilation, per-source metadata `HeadObject` fetches (both modes), manifest insertion in copy mode, `collect_zip_entry_plans`, and the planned-entries accounting. In copy mode (`extract:false`) no ZIP stages run, so all four sub-timings report 0 and `plan` is the `HeadObject` round-trips plus that CPU-only residual; the zeros are the honest answer there, not an omission.
