...
For previous-object deletion, omitting `onChange.fromBucket` reuses the current `destinationBucket`; an explicit `fromBucket` authorizes a changed previous bucket. An unchanged current distribution is invalidated automatically. A changed previous distribution must be passed to `onChange.invalidateDistribution` so CDK can grant distribution-specific invalidation permissions and synthesize its dependency. Object deletion and previous-distribution invalidation remain independent actions: omitting the latter does not block requested object deletion, and the provider logs that it skipped an unauthorized previous-distribution invalidation.
